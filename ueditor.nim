import os, re, jester, asyncdispatch, htmlgen, asyncnet,json,times

let configStr = """/* 前后端通信相关的配置,注释只允许使用多行方式 */
                {
                    /* 上传图片配置项 */
                    ...
                    "snapscreenUrlPrefix": "/upload/", /* 图片访问路径前缀 */
...
proc GetParamRoutes*(request:Request):string = 
...
proc PostParamRoutes*(request:Request):string =
    case request.params["action"]
    of "uploadimage":
